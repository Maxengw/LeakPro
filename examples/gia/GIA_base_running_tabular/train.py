...
def log_classification(epoch: int, epochs: int, lr: float, train_stats: Dict, val_stats: Dict, is_multiclass: bool) -> None:
    """Classification type logs, binary and multiclass."""
    if is_multiclass:
...
def train(
    ...
    for epoch in range(epochs):
        ...
        train_stats = run_epoch(
            loaders["train_loader"], model, criterion, is_binary, is_multiclass, metric_classes, optimizer, device
        )
...
def train_global_model(cfg: dict) -> Tuple[Path, float]:
    """Function for training the global model."""
    ...
    _ = num_classes == 1 and next(iter(train_loader))[1].dtype != torch.long
    out_classes = 1 if is_binary else num_classes
    metric_classes = 2 if is_binary else num_classes

    logger.info(
        "Detected task=%s, num_classes=%s",
        "binary" if is_binary else ("multiclass" if is_multiclass else "regression"),
        num_classes,
    )
