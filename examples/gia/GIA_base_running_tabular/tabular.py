...
def _load_dataset_meta(meta_path: Optional[Path]) -> dict[str, Any]:
    """Load dataset meta."""
...
def _fit_encoders(x_train: pd.DataFrame, num_cols: list[str], cat_cols: list[str], cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    cat_dummy_cols = (
        pd.get_dummies(x_train[cat_cols].fillna(missing_token), drop_first=False).columns.tolist() if cat_cols else []
    )
    cat_categories = {c: pd.Categorical(x_train[c].fillna(missing_token), ordered=False).categories.tolist() for c in cat_cols}
    return {
        "num_cols": num_cols,
        "cat_cols": cat_cols,
...
def _apply_encoders(x: pd.DataFrame, meta: dict[str, Any], cfg: dict[str, Any]) -> pd.DataFrame:
    ...
    return (
        ...
        else (x_num if len(x_num.columns) else x_cat)
    )


def _encode_target_train(
    y: pd.Series, mode: str, provided_classes: list[Any] | None = None
) -> tuple[torch.Tensor, int, list[Any] | None, str]:
...
def _read_frame(path: Path, target_col: str | int, no_header: bool, read_kwargs: dict[str, Any]) -> pd.DataFrame:
...
def _resolve_test_path(data_path: Path, test_path: str | Path) -> Path:
...
def _impute_features(df: pd.DataFrame, num_cols: list[str], cat_cols: list[str], missing_values: object) -> pd.DataFrame:
...
def _prepare_target(y: pd.Series, missing_values: object) -> tuple[pd.Series, str, list[Any] | None]:
...
def _split_global_pool(
    ...
    """Split global pool into train/val and optionally test when no external split is provided."""
    if use_external_test:
        train_idx, val_idx = train_test_split(
            y_global.index, test_size=0.15, random_state=seed, stratify=None if is_regression else y_global
        )
    ...
    train_idx, temp_idx = train_test_split(
        y_global.index, test_size=1 - train_frac, random_state=seed, stratify=None if is_regression else y_global
    )
    x_train, y_train = x_global.loc[train_idx], y_global.loc[train_idx]
    ...
    val_idx, test_idx = train_test_split(
        y_temp.index, test_size=1 - val_ratio, random_state=seed, stratify=None if is_regression else y_temp
    )
...
def _load_frames_and_meta(
    cfg: dict[str, Any],
) -> tuple[pd.DataFrame, pd.DataFrame | None, dict[str, Any], str | int, object]:
    """Load CSVs and dataset meta; return (df, test_df, meta, target_col, missing_values)."""
    ...
    meta = _load_dataset_meta(cfg.get("meta_path"))
    ...
    test_df = None
    if (test_path := meta.get("has_test_split")) is not None:
    ...
    return df, test_df, meta, target_col, missing_values
...
def load_tabular_dataset(
    ...
    df, test_df, meta, target_col, missing_values = _load_frames_and_meta(cfg)
    cfg = {**cfg, "missing_token": missing_values}

    y, x = df[target_col], df.drop(columns=[target_col])
    y_test, x_test = (test_df[target_col], test_df.drop(columns=[target_col])) if test_df is not None else (None, None)
    ...
    y_test = (
        y_test.fillna(missing_values)
        if (y_test is not None and mode == "classification")
        else (pd.to_numeric(y_test, errors="coerce").fillna(0) if y_test is not None else None)
    )

    _log_target_stats(y, str(target_col), mode)

    logger.info("Loaded data: %d rows, %d cols (after minimal imputation)", df.shape[0], df.shape[1])

    is_regression, seed, client_frac = mode == "regression", cfg.get("seed", 42), cfg.get("client_frac", 0.1)
    global_idx, client_idx = train_test_split(
        y.index, test_size=client_frac, random_state=seed, stratify=None if is_regression else y
    )
    ...
    x_train, y_train, x_val, y_val, x_split_test, y_split_test = _split_global_pool(
        x_global,
        y_global,
        seed,
        is_regression,
        cfg.get("train_frac", 0.70),
        cfg.get("val_frac", 0.15),
        test_df is not None,
    )
    if test_df is None:
        x_test, y_test = x_split_test, y_split_test
    ...
    # encode targets
    y_train_t, num_classes, target_classes, target_mode = _encode_target_train(
        y_train, mode, provided_classes=target_classes_full
    )
...
def get_tabular_loaders(
    ...
    """Create DataLoaders and metadata for tabular data. Returns (splits_dict, encoder_meta)."""
    ...
    loaders: dict[str, Any] = {
        ...
        "val_loader": DataLoader(result["val_ds"], batch_size=cfg.get("batch_size", 64), shuffle=False)
        if result["val_ds"] is not None
        else None,
        "test_loader": DataLoader(result["test_ds"], batch_size=cfg.get("batch_size", 64), shuffle=False)
        if result["test_ds"] is not None
        else None,
